...
const objBusqueda = { //-> el evento "change" seteara este objeto , evento que tiene asociada (se llamo) a la funcion getValue
    ...
    amount.addEventListener('input', getValue);
...
function submitForm(e){
    e.preventDefault();
    const {moneda, criptomoneda, amount} = objBusqueda;
    if (moneda === '' || criptomoneda === '' || amount === '') {
...
function mostrarCotizacion(data){
    clearHTML();
    const {amount} = objBusqueda;
    
    const {PRICE, HIGHDAY, LOWDAY, CHANGEPCT24HOUR, LASTUPDATE} = data;
    const answer = document.createElement('div');
    answer.classList.add('display-info');
    let resultado =  parseFloat(amount * PRICE);
    answer.innerHTML = `
        <p class="main-price">Resultado: <span>${resultado}</span></p>
